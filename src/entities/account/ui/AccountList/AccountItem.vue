...
import {
  type Account,
  AccountTypes,
} from "@src/entities/account/types/account.types";
import {
...
const checkRequiredFields = () => {
  ...
  for (const field of requiredFields) {
    if (
      !props.account[field] &&
      !(field === "password" && props.account.type === AccountTypes.LDAB)
    ) {
...
const checkErrorStatus = (field: keyof Account) =>
  showRequiredFieldsError.value && !props.account[field];
</script>
...
<template>
  <tr>
    ...
    <td :colspan="account.type === AccountTypes.LDAB ? 2 : 1">
      <AccountValueInput
        :required-error="checkErrorStatus('value')"
        @onChange="(value) => checkRequiredFields()"
        class="w-full"
